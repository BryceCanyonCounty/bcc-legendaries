...
function npc()
    ...
    local blip = Citizen.InvokeNative(0x45F13B7E0A15C880, -1282792512, Npcblipcoords.x, Npcblipcoords.y, Npcblipcoords.z,
        100.0)
    ...
    while true do                                                              --creates loop
        ...
        if StopAll == false then                                               --if the variable from the function offcatch is false(player not dead) then
            ...
            while distancetracker == true do              --creates a loop
                ...
                local dist = GetDistanceBetweenCoords(pl.x, pl.y, pl.z, Npcblipcoords.x, Npcblipcoords.y, Npcblipcoords
                    .z)                                   --gets dist between player and global varible set in menusetup.lua
                if dist < 200 then                        --if distance is less than 200
                    Nrange = 1
                    break                                 --sets variable to 1 and breaks loop
                end
            end
            if Nrange == 1 then          --if varaible is 1 then
                if check == false then   --if the catch varaible is false then do
                    ...
                    RemoveBlip(blip)
                    Nrange = Nrange +
                        1                                                                       --makes nrange 2 prevents it from running more than once
                    for k, v in pairs(Npccoords) do                                             --opens npccoords table
                        ...
                        count[k] = createdped
                            [k]                                                                 --sets count[k] to createped[k] allowing us to in the for loop below acces the created ped table while still using the data in the table
                        ...
                        Citizen.CreateThread(function()                                         -- creates a thread
                            local x = #
                                Npccoords                                                       --sets x = to the table coords
...
function npcsearch1()
    ...
    local firstprompt = PromptGroup:RegisterPrompt(Config.Language.ChestPrompt, 0x760A9C6F, 1, 1, true, 'hold',
        { timedeventhash = "MEDIUM_TIMED_EVENT" })                                                        --creates a prompt in the prompt group
            ...
            if Searchinrange2 == 1 then                                                              --if the trigger is 1 then
                ...
                if firstprompt:HasCompleted() then                                                   --if the prmpt has been done then
                    ...
                    Citizen.CreateThread(function()                                                  --Starts the play animation setup(This entire thread is needed)
                        ...
                        TaskPlayAnim(PlayerPedId(), 'mech_ransack@chest@med@open@crouch@b', 'base', 8.0, 8.0, 1000, 17,
                            0.2, false, false, false) --plays the animation
                    end)                              --end of animation setup
                    VORPcore.NotifyRightTip(Config.Language.Cluefound, 6000)
